...
class WikipediaJapanSource extends JapanSource {
  const WikipediaJapanSource(this._client);
  ...
  @override
  Future<JapanEntry?> entryFor(String seedKey) async {
    // Try a couple of categories, in case the first is having a bad day.
    for (var attempt = 0; attempt < 3; attempt++) {
      final category = JapanCategory.all[
          dailyIndex(seedKey, 'japan-category-$attempt', JapanCategory.all.length)];
      ...
      for (var i = 0; i < titles.length && i < _maxCandidates; i++) {
        ...
        final entry = await _summaryOf(title, category.label);
  ...
  Future<List<String>> _membersOf(JapanCategory category) async {
    final result = await _client.getJson<List<String>>(
      ...
      parse: (json) {
        ...
        final members = (map['query'] as Map<String, dynamic>?)?['categorymembers']
            as List<dynamic>?;
  ...
  Future<JapanEntry?> _summaryOf(String title, String categoryLabel) async {
    final result = await _client.getJson<JapanEntry?>(
    ...
    return switch (result) {
      Success<JapanEntry?>(:final data) => data,
      Failure<JapanEntry?>() => null,
    };
  }
}

...
void main() {
  ...
  test('edits fields without disturbing the others', () async {
    ...
    final updated =
        await (db.select(db.todos)..where((t) => t.id.equals(todo.id)))
            .getSingle();
    expect(updated.title, 'Renamed');
    expect(updated.description, 'Note',
        reason: 'omitting an argument must leave the column alone');
    expect(updated.updatedAt.isAfter(todo.createdAt.subtract(const Duration(seconds: 1))), isTrue);
  });
  ...
  test('sets and clears a due date', () async {
    ...
    var updated = await (db.select(db.todos)..where((t) => t.id.equals(todo.id)))
        .getSingle();
    expect(updated.dueDate, due);

    await repository.edit(todo.id, clearDueDate: true);
    updated = await (db.select(db.todos)..where((t) => t.id.equals(todo.id)))
        .getSingle();
  ...
  test('the stream emits on every change', () async {
    final emissions = <int>[];
    final sub = repository.watchAll().listen((rows) => emissions.add(rows.length));

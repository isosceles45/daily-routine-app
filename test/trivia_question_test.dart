...
void main() {
  ...
  test('answer order is stable for a date but differs across dates', () {
    ...
    for (var d = 1; d <= 28; d++) {
      ...
      orderings
          .add(TriviaQuestion.fromOpenTdb(openTdbResult(), date: day).answers.join('|'));
    }
  ...
  test('rejects a result missing its fields', () {
    expect(
      () => TriviaQuestion.fromOpenTdb(
          openTdbResult(question: ''), date: date),
      throwsFormatException,
    );
    expect(
      () => TriviaQuestion.fromOpenTdb(
          {...openTdbResult(), 'incorrect_answers': <String>[]},
          date: date),
      throwsFormatException,
    );
  });
  ...
  test('derives a stable id from the question text', () {
    expect(TriviaQuestion.idFor('Same text'), TriviaQuestion.idFor('Same text'));
